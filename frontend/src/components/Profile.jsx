    ...
    <section className="rounded-lg bg-white p-4 shadow-soft ring-1 ring-slate-100 sm:p-5">
      ...
      <form onSubmit={handleSubmit} className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <label className="space-y-1">
          ...
          <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" className="w-full rounded-lg border-0 bg-slate-100 px-4 py-3" />
        </label>

        <div className="flex gap-2 sm:col-span-2">
          <button type="submit" disabled={loading} className="flex-1 rounded-lg bg-violet-600 px-4 py-2 text-white">{loading ? "Enregistrement..." : "Enregistrer"}</button>
        </div>
      </form>
    </section>
  );
